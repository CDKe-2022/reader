...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!url.pathname.startsWith('/api/')) {
      ...
      return new Response(assetResponse.body, { status: assetResponse.status, statusText: assetResponse.statusText, headers: newHeaders });
    }
    ...
    try {
      ...
      if (path === '/api/books' && request.method === 'GET') {
        const { results } = await db.prepare(`
          SELECT id, name, word_count, total_chapters, total_paragraphs, progress_gidx, current_chapter_title, import_time, sort_order
      ...
      if (path === '/api/books' && request.method === 'POST') {
        ...
        if (!bookId) {
          return new Response(JSON.stringify({ error: 'Missing X-Book-Id header' }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
        }
        // 文件大小限制检查 (50MB，Cloudflare Free 计划上限 100MB)
        const contentLength = parseInt(request.headers.get('Content-Length') || '0');
        if (contentLength > 50 * 1024 * 1024) {
          return new Response(JSON.stringify({ error: 'File too large (Max 50MB)' }), { status: 413, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
        }
      ...
      if (path === '/api/books/meta' && request.method === 'POST') {
        ...
        if (existing) {
          // 更新已有记录
          await db.prepare(`
            UPDATE books SET name = ?, word_count = ?, total_chapters = ?, total_paragraphs = ?, ch_map = ?, current_chapter_title = ?, import_time = ?, sort_order = ? WHERE id = ?
          `).bind(
            metadata.name, metadata.wordCount, metadata.totalChapters, metadata.totalParagraphs, 
            metadata.chMap, metadata.firstChapterTitle || '开始阅读', Date.now(), Date.now(), bookId
          ).run();
        } else {
          // 新增记录
          await db.prepare(`
            INSERT INTO books (
              id, name, r2_key, word_count, total_chapters, total_paragraphs, ch_map, current_chapter_title, import_time, sort_order
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).bind(
            bookId, metadata.name, r2_key, metadata.wordCount, metadata.totalChapters, metadata.totalParagraphs, 
            metadata.chMap, metadata.firstChapterTitle || '开始阅读', Date.now(), Date.now()
          ).run();
        }
        return Response.json({ success: true, id: bookId }, { headers: corsHeaders });
      }
      ...
      if (bookMatch && request.method === 'GET') {
        ...
        const book = await db.prepare(`
          SELECT id, name, r2_key, word_count, total_chapters, total_paragraphs, progress_gidx, ch_map, current_chapter_title
          FROM books WHERE id = ?
        `).bind(id).first();
      ...
      if (bookMatch && request.method === 'DELETE') {
        ...
        // [Bug 3 修复] 补充 Content-Type: application/json 避免前端解析异常
        if (!book) return new Response(JSON.stringify({ error: 'Book not found' }), { status: 404, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
      ...
      if (contentMatch && request.method === 'GET') {
        ...
        const book = await db.prepare('SELECT r2_key FROM books WHERE id = ?').bind(id).first();
        // [Bug 2 修复] 补充 corsHeaders 避免前端跨域拦截报错
        if (!book) return new Response('Not found', { status: 404, headers: corsHeaders });
        const obj = await bucket.get(book.r2_key);
        if (!obj) return new Response('File not found in R2', { status: 404, headers: corsHeaders });
        return new Response(obj.body, { headers: { 'Content-Type': 'text/plain; charset=utf-8', ...corsHeaders } });
      }
      ...
      if (progressMatch && request.method === 'PUT') {
        ...
        await db.prepare(`
          UPDATE books SET progress_gidx = ?, current_chapter_title = ? WHERE id = ?
      ...
      if (path === '/api/tts-proxy' && request.method === 'POST') {
        ...
        if (body._model) headers['model'] = body._model;
        ...
        proxyHeaders.set('Content-Type', upstreamRes.headers.get('Content-Type') || 'audio/mpeg');
      ...
      return new Response(JSON.stringify({ error: err.message, stack: err.stack }), { status: 500, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
    }
  }
};
